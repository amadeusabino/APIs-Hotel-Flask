...
# path /hoteis?cidade=Campinas&estrelas_min=4&diaria_max=400
...
class Hotel(Resource):
    ...
    atributos.add_argument('nome', type=str, required=True, help="The field must be informed.")
    ...
    def get(self, hotel_id):
        ...
        if hotel:
            return hotel.json()
        return{'message': 'Hotel_id não encontrado'}, 404     # not found
    ...
    def post(self, hotel_id):
        ...
        if dados.get('nome').strip() == '':
            return {"message": "O Campo 'nome' não pode ficar sem valor."}

        hotel = HotelModel(hotel_id, **dados)

        if not SiteModel.find_by_id(dados['site_id']):
            return {'message': 'The hotel must be linked to a site'}, 400
